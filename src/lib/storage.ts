...
function cleanBackground(value: unknown): SlideBackground | undefined {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Partial<SlideBackground>;
  if (raw.mode !== "theme" && raw.mode !== "solid" && raw.mode !== "gradient") return undefined;
  const background: SlideBackground = {
    mode: raw.mode,
    ...(typeof raw.decorations === "boolean" ? { decorations: raw.decorations } : {}),
  };
  if (raw.mode === "solid" || raw.mode === "gradient") {
    background.color = isHexColor(raw.color) ? raw.color : "#F8FAFC";
  }
  if (raw.mode === "gradient") {
    background.color2 = isHexColor(raw.color2) ? raw.color2 : "#DBEAFE";
    background.angle =
      typeof raw.angle === "number" && Number.isFinite(raw.angle)
        ? Math.max(0, Math.min(360, raw.angle))
        : 160;
  }
  return background;
}
...
function cleanTypography(value: unknown): SlideTypography | undefined {
  const base = cleanCaptionTypography(value);
  if (!value || typeof value !== "object") return base;
  const raw = value as Partial<SlideTypography>;
  const locales =
    raw.locales && typeof raw.locales === "object"
      ? Object.fromEntries(
          Object.entries(raw.locales)
            .map(([locale, typography]) => [locale, cleanCaptionTypography(typography)])
...
function migrateSlide(slide: Slide): Slide {
  ...
  return {
    ...
    ...(typeof slide.fontFamily === "string" ? { fontFamily: slide.fontFamily } : { fontFamily: undefined }),
