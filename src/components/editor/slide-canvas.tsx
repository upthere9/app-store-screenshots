...
function captionTypographyFor(slide: Slide, locale: string, unit: number) {
  const base = slide.typography || {};
  const localeTypography: CaptionTypography = base.locales?.[locale] || {};
  const defaultLabelSize = unit * DEFAULT_LABEL_FONT_SIZE_RATIO;
  const defaultHeadlineSize = unit * DEFAULT_HEADLINE_FONT_SIZE_RATIO;

  return {
    labelFontFamily:
      localeTypography.labelFontFamily ||
      base.labelFontFamily ||
      slide.fontFamily ||
      "system",
    headlineFontFamily:
      localeTypography.headlineFontFamily ||
...
function backgroundFor(theme: Theme, inverted?: boolean, background?: Slide["background"]) {
  if (background?.mode === "solid" && background.color) {
    return background.color;
  }
  if (background?.mode === "gradient" && background.color && background.color2) {
    return `linear-gradient(${background.angle ?? 160}deg, ${background.color} 0%, ${background.color2} 100%)`;
  }
  if (inverted) {
...
function SlideBackground({
  ...
  const showDecorations = slide.background?.decorations !== false;
  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        overflow: "hidden",
        background: backgroundFor(theme, inverted, slide.background),
...
function FeatureGraphicCanvas({
  ...
  const family = fontStack(slide.fontFamily);
  const customBackground = slide.background && slide.background.mode !== "theme";
  const fg = customBackground && !slide.inverted ? theme.fg : theme.fgAlt;
  const showDecorations = slide.background?.decorations !== false;
  return (
    <div
      style={{
        ...
        position: "relative",
        overflow: "hidden",
        background: backgroundFor(theme, true, slide.background),
        display: "flex",
        alignItems: "center",
    ...
    >
      {showDecorations && <Blob cW={cW} color={theme.accent} x={70} y={20} size={50} opacity={0.45} />}
      <div style={{ display: "flex", alignItems: "center", gap: cW * 0.03, zIndex: 2 }}>
        ...
        <div>
          ...
          <EditableText
            ...
            style={{
              fontSize: cW * 0.028,
              color: customBackground && !slide.inverted ? theme.fg : "rgba(255,255,255,0.85)",
              marginTop: cW * 0.012,
              lineHeight: 1.25,
            }}
          />
...
function SlideElements({
  ...
  function renderTextElement(textElement: TextElement, index: number) {
    ...
    const family = fontStack(textElement.fontFamily || slide.fontFamily);
    return (
      <Movable
        key={textElement.id}
        rect={toGlobal(rect)}
