...
type Props = {
  ...
  onImportLocalization: (payload: unknown) => void;
  onDownloadLocalization: () => void;
  device: Device;
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 border-b bg-card/40 px-4 py-2">
      ...
      <LocalizationDialog
        ...
        setLocales={props.setLocales}
        onImportLocalization={props.onImportLocalization}
...
function LocalizationDialog({
  ...
  locales,
  setLocales,
  onImportLocalization,
  onDownloadLocalization,
  busy,
}: {
  open: boolean;
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      onImportLocalization(JSON.parse(text));
      setImportError(null);
      onOpenChange(false);
    } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Manage export locales and import slide copy.</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Locale codes</label>
            <Input
              value={draft}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Active locale</label>
            <Select value={locale} onValueChange={setLocale} disabled={busy || locales.length === 0}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-wrap justify-between gap-2">
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={onDownloadLocalization}
                disabled={busy}
              >
                <Download className="h-4 w-4" />
                Download JSON
